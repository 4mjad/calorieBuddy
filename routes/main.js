module.exports = function(app) // this file has been exported as a function
{
   ...
   const redirectLogin = (req, res, next) => { // route redirect which checks if user is/isn't logged in - pages with access control will require users to login before viewing them
    ...
    app.get('/search-result', function (req, res) { // get route for search results
      ...
      if(err) throw err; // error handling
       ...
       db.collection('food').find({name:{$regex:new RegExp(req.query.keyword,"i")}}).toArray((findErr, results) => { // finds name of foods with specific key words - uses regular expressions for matching 		the text
   ...
   app.post('/updatedfood', [
        ...
        ], function (req,res) { // POST route for updated food
        const errors = validationResult(req); // gets the validation errors from a request
        if (!errors.isEmpty()) { // if there are errors then redirect back to the page
        res.redirect('./updatedfood'); }
	...
	db.collection('food').findOne({name:req.body.name},
        ...
        if (err) throw err;
        if (results != null) { // if results are true and username isnt the same as the person who added the item, then user cannot update the item
        if (results.username != req.session.userId){
	...
	{$set: { value: req.sanitize(req.body.value), // $set updates the values and then stores into the variables                                                                                                   
	unit: req.sanitize(req.body.unit),     // sanitize ensures nothing unnecessary is entered  
        calories: req.sanitize(req.body.calories),                                                                                                                                                           	    carbs: req.sanitize(req.body.carbs),                                                                                                                                                             
        fat: req.sanitize(req.body.fat),                                                                                                                                                        
        protein: req.sanitize(req.body.protein),                                                                                                                                                                    salt: req.sanitize(req.body.salt),                                                                      
  ...
  app.post('/deletedfood', function (req,res) { //POST method route
	...
	function (findErr, results) {
	if (err) throw err;
	if (results != null) { // if results are true and username isnt the same as the person who added the item, then user cannot delete the item
	if (results.username != req.session.userId){
